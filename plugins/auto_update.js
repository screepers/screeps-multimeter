...
const RELEASES_URI = 'https://api.github.com/repos/CGamesPlay/screeps-multimeter/releases';
const USER_AGENT = 'screeps-multiplayer auto-updater';
const VERSION = require('../package.json').version;

function checkForUpdates() {
  return rp({ uri: RELEASES_URI, headers: { 'User-Agent': USER_AGENT }, json: true })
    .then((releases) => {
      ...
      if (releases.length > 0) {
        return {
          ...
          notes: _.map(releases, (r) => "Release notes for " + r.tag_name + ":\n" + r.body.replace(/\r/g, '')).join("\n\n"),
        };
      } else {
...
module.exports = function(multimeter) {
  ...
  function banner() {
    return "There is a new version of Multimeter!{/} This is " + release.current + ", the latest is {bold}" + release.latest + "{/}.";
  }
    ...
    handler: () => {
      if (release) {
        if (release.current != release.latest) {
          multimeter.log(banner() + "\n\n" + release.notes + "\n\nUse npm to update to the latest version.")
        } else {
          multimeter.log("This is Multimeter " + VERSION + ", which is the latest version.");
        }
      } else {
